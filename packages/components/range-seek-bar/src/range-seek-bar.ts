...
import type { QTRangeSeekBarEvent } from './range-seek-types'

const RangeSeekBar = defineComponent({
  name: 'RangeSeekBarView',
  props: rangeSeekBarProps,
  emits: ['onFocusChanged', 'onSeekChanged'],
  setup(props, context) {
    const viewRef = ref()
    ...
    function setProgress(progress: number) {
      Native.callUIFunction(viewRef.value, 'setProgress', [progress], () => {
        //
      })
    }
    ...
    function setMaxProgress(progress: number) {
      Native.callUIFunction(viewRef.value, 'setMaxProgress', [progress], () => {
        //
      })
    }
    ...
    const render = () => {
      return h('ESHorizontalSeekBarViewComponent', {
        ref: viewRef,
        ...
        onSeekBarChange: (evt: QTRangeSeekBarEvent) => {
