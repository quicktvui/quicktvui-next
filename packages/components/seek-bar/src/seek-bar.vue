...
import SeekBarView from './seek-bar'
import type { QTGradient } from '../../gradient'

export default defineComponent({
  name: 'QtSeekBar',
  components: { SeekBarView },
  props: seekBarProps,
  emits: ['onSeekChanged', 'onFocusChanged'],
  setup(props, context) {
    const viewRef = ref()
    const ns = useNamespace('seek-bar')

    //style
    const seekBarKls = computed(() => [ns.b()])
    ...
    function resetSeekbar() {
      viewRef.value?.resetSeekbar()
    }

    function startSeek(forward: boolean) {
      viewRef.value?.startSeek(forward)
    }

    function stopSeek() {
      viewRef.value?.stopSeek()
    }

    function isFocused() {
      return viewRef.value?.isFocused()
    }
