...
import { ESKeyCode, ESLogLevel, useESLog } from '@extscreen/es3-core'
  ...
  setup() {
    ...
    function onFocusChange(event) {
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          `${event.isFocused}-----onFocusChange------>>>>>${focused.value}`
        )
      }
    }
    ...
    function initESSeekBar() {
      ...
      seekBarRef.value?.setThumbUrl(
        'https://qcloudimg-moss.cp47.ott.cibntv.net/channelzero_image/2023/02/08/40a9ad3c-ec51-41b3-b144-a215b2e809a5.png'
      )
      seekBarRef.value?.setThumbInactivatedUrl(
        'https://qcloudimg-moss.cp47.ott.cibntv.net/channelzero_image/2023/02/08/6115d0e3-5f59-4d36-be44-95cca74caf37.png'
      )
    ...
    function onKeyDown(keyEvent) {
      if (
        keyEvent.keyCode === ESKeyCode.ES_KEYCODE_DPAD_LEFT ||
