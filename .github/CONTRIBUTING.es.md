# Guía para Contribuidores a `QuickTVUI`

¡Hola! Gracias por elegir [QuickTVUI](http://element.eleme.io/#/en-US).

`QuickTVUI` es un archivo de componentes para desarrolladores y para gerentes de productos ‘web’ basado en [Vue 3](https://vuejs.org/)

Estamos orgullosos de que usted esta interesado en contribuir al proyecto `QuickTVUI`. Antes de someter sus contribuciones, por favor tome un momentito para leer estas simples guías para contribuidores.
...
- Por favor especifique que versión de `QuickTVUI` y `Vue` que esta utilizando, y que versión de sistema operativo y que versión de navegador web que está utilizando. [Playground](https://element-plus.run/) esta recomendado para crear un entorno para reproducir el problema claramente.
