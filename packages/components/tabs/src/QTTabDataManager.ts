...
import type { QTListViewItemState } from '../../list-view'
import type { QTWaterfallItem, QTWaterfallSection } from '../../waterfall'
import type { QTTab, QTTabIndex, QTTabPageDataState } from './tabs-types'
...
export function createQTTabDataManager(): QTTabDataManager {
  ...
  function setPageDataState(
    pageIndex: number,
    pageData: QTTabPageDataState
  ): void {
    pageMap.set(pageIndex, pageData)
  }

  function getTabPageDataState(
    pageIndex: number
  ): QTTabPageDataState | undefined {
  ...
  function insertSectionList(
    ...
    const previousItemCount = getRangeSectionListItemCount(
      sectionList,
      0,
      sectionListLength
    )
  ...
  function deleteSection(
    ...
    const deleteItemCount = getRangeSectionListItemCount(
      sectionList,
      sectionIndex,
      count
    )
  ...
  function getSectionListItemCount(
    sections: Array<QTWaterfallSection>
  ): number {
