<template>
  <div class="es-sdk-root-css">
    ...
    <div class="es-sdk-content-row-css">
      <s-text-button
        text="FIT_PARENT"
        @onButtonClicked="setAspectRatioFitParent"
      />
      <s-text-button
        text="FILL_PARENT"
        @onButtonClicked="setAspectRatioFillParent"
      />
      <s-text-button
        text="WRAP_CONTENT"
        @onButtonClicked="setAspectRatioWrapContent"
      />
      <s-text-button
        text="MATCH_PARENT"
        @onButtonClicked="setAspectRatioMatchParent"
      />
      <s-text-button
        text="16_9_FIT_PARENT"
        @onButtonClicked="setAspectRatio169FitParent"
      />
      <s-text-button
        text="4_3_FIT_PARENT"
        @onButtonClicked="setAspectRatio43FitParent"
      />
    </div>
  </div>
</template>
...
import { ESPlayerAspectRatio } from '@extscreen/es3-player'
import type {
  ESIPlayerManager,
  ESMediaItem,
  ESMediaItemList,
...
export default defineComponent({
  ...
  setup() {
    ...
    function setAspectRatioFitParent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_ASPECT_FIT_PARENT
      )
    }

    function setAspectRatioFillParent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_ASPECT_FILL_PARENT
      )
    }

    function setAspectRatioWrapContent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_ASPECT_WRAP_CONTENT
      )
    }

    function setAspectRatioMatchParent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_ASPECT_MATCH_PARENT
      )
    }

    function setAspectRatio169FitParent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_16_9_FIT_PARENT
      )
    }

    function setAspectRatio43FitParent() {
      playerManager.value?.setAspectRatio(
        ESPlayerAspectRatio.ES_PLAYER_AR_4_3_FIT_PARENT
      )
    }
