...
import { epOutput } from '@quicktvui/build-utils'
    ...
    cssnano({
      preset: [
        'default',
        {
          ...
          //Collapses longhand properties into the shorthand representation, and where possible will also collapse top/right/bottom/left values.
          //Supports margin, padding & border longhands.
          mergeLonghand: false,
        },
      ],
    }),
    ...
    .pipe(
      rename((path) => {
        if (!noElPrefixFile.test(path.basename)) {
          path.basename = `qt-${path.basename}`
        }
      })
    )
