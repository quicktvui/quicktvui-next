...
import { errorAndExit, getWorkspacePackages } from '@quicktvui/build-utils'
...
async function main() {
  ...
  consola.debug(chalk.yellow(`Updating package.json for quicktvui`))

  const pkgs = Object.fromEntries(
    (await getWorkspacePackages()).map((pkg) => [pkg.manifest.name!, pkg])
  )
  const elementPlus = pkgs['quicktvui'] || pkgs['@quicktvui/nightly']
  const eslintConfig = pkgs['@quicktvui/eslint-config']
  const metadata = pkgs['@quicktvui/metadata']
