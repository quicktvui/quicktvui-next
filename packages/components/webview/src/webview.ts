...
import type { QTWebViewEvent } from './webview-types'

const WebView = defineComponent({
  name: 'QtWebView',
  emits: [
    'onCanGoBack',
    'onCanGoForward',
  ...
  setup(props, context) {
    ...
    const render = () => {
      return h('ESWebViewComponent', {
        onPageStarted: (evt: QTWebViewEvent) => {
          const url = evt.url
          context.emit('onPageStarted', url)
        },
        onPageFinished: (evt: QTWebViewEvent) => {
          const url = evt.url
          context.emit('onPageFinished', url)
        },
        onLoadResource: (evt: QTWebViewEvent) => {
          const url = evt.url
          context.emit('onLoadResource', url)
        },
        onReceivedSslError: (evt: QTWebViewEvent) => {
          context.emit('onReceivedSslError', evt)
        },
        onReceivedError: (evt: QTWebViewEvent) => {
        ...
        onShouldOverrideUrlLoading: (evt: QTWebViewEvent) => {
          const url = evt.url
          context.emit('shouldOverrideUrlLoading', url)
        },
        onJs2Vue: (evt: QTWebViewEvent) => {
