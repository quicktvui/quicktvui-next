...
import { ESIPlayer } from '@extscreen/es3-player'
import type {
  ESIPlayerManager,
  ESMediaItem,
  ESMediaItemList,
...
export default defineComponent({
  ...
  setup() {
    ...
    function onPlayerPlayMediaSourceList(
      mediaSourceList: ESMediaSourceList
    ): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayMediaSourceList:${JSON.stringify(mediaSourceList)}\n`
    }

    function onPlayerPlayMediaSource(mediaSource: ESMediaSource): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayMediaSource:${JSON.stringify(mediaSource)}\n`
    }

    function onPlayerInterceptSuccess(value: ESPlayerInterceptResult): void {
      contentText.value = `${
        contentText.value
      }onPlayerInterceptSuccess:${JSON.stringify(value)}\n`
    }

    function onPlayerInterceptError(value: ESPlayerInterceptError): void {
      contentText.value = `${
        contentText.value
      }onPlayerInterceptError:${JSON.stringify(value)}\n`
    }
    ...
    function onPlayerError(error: ESPlayerError): void {
      contentText.value = `${contentText.value}onPlayerError:${JSON.stringify(
        error
      )}\n`
    }

    function onPlayerInfo(info: ESPlayerInfo): void {
      contentText.value = `${contentText.value}onPlayerInfo:${JSON.stringify(
        info
      )}\n`
    }
    ...
    function onPlayerDefinitionListChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerDefinitionListChanged:${JSON.stringify(definitionList)}\n`
    }

    function onPlayerDefinitionChanged(definition: ESPlayerDefinition): void {
      contentText.value = `${
        contentText.value
      }onPlayerDefinitionChanged:${JSON.stringify(definition)}\n`
    }

    function onPlayerPlayRateListChanged(rateList: Array<ESPlayerRate>): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayRateListChanged:${JSON.stringify(rateList)}\n`
    }

    function onPlayerPlayRateChanged(rate: ESPlayerRate): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayRateChanged:${JSON.stringify(rate)}\n`
    }
    ...
    function onPlayerDecodeListChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerDecodeListChanged:${JSON.stringify(decodeList)}\n`
    }

    function onPlayerDecodeChanged(decode: ESPlayerDecode): void {
      contentText.value = `${
        contentText.value
      }onPlayerDecodeChanged:${JSON.stringify(decode)}\n`
    }
    ...
    function onPlayerAspectRatioListChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerAspectRatioListChanged:${JSON.stringify(aspectRatioList)}\n`
    }
    ...
    function onPlayerAspectRatioChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerAspectRatioChanged:${JSON.stringify(aspectRatio)}\n`
    }
    ...
    function onPlayerPlayMediaSourceListModeListChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerPlayMediaSourceListModeListChanged:${JSON.stringify(modeList)}\n`
    }
    ...
    function onPlayerPlayMediaSourceListModeChanged(
      ...
      contentText.value = `${
        contentText.value
      }onPlayerPlayMediaSourceListModeChanged:${JSON.stringify(mode)}\n`
    }
    ...
    function onPlayerPlayMediaList(playList: ESMediaItemList): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayMediaList:${JSON.stringify(playList)}\n`
    }

    function onPlayerPlayMedia(mediaItem: ESMediaItem): void {
      contentText.value = `${
        contentText.value
      }onPlayerPlayMedia:${JSON.stringify(mediaItem)}\n`
    }
    ...
    function onPlayerWindowTypeChanged(windowType: ESPlayerWindowType): void {
      contentText.value = `${
        contentText.value
      }onPlayerWindowTypeChanged:${JSON.stringify(windowType)}\n`
    }
    ...
    function onKeyDown(keyEvent: ESKeyEvent): boolean {
      contentText.value = `${contentText.value}onKeyDown:${JSON.stringify(
        keyEvent
      )}\n`
      return true
    }
    ...
    function onKeyUp(keyEvent: ESKeyEvent): boolean {
      contentText.value = `${contentText.value}onKeyUp:${JSON.stringify(
        keyEvent
      )}\n`
      return true
    }
