import { QTMediaSeriesStyleType, QTMediaSeriesType } from './series-types'
import type {
  QTMediaSeries,
  QTMediaSeriesData,
  QTMediaSeriesGroup,
...
export function buildGroupParams(
  ...
  if (group.enable) {
    ...
    if (groupStyle) {
      if (groupStyle.groupMarginLeft)
        groupParams.groupMarginLeft = groupStyle.groupMarginLeft
    ...
    if (styleType == QTMediaSeriesStyleType.QT_MEDIA_SERIES_STYLE_TYPE_VIP) {
      ...
      if (groupStyle.focusBackground)
        groupParams.focusBackground = groupStyle.focusBackground
...
export function buildCommonParams(
  ...
  const contentHeight = buildMediaSeriesContentHeight(
    type,
    group,
    styleType,
    data
...
export function buildLeftRightParams(
  type: QTMediaSeriesType,
  group: QTMediaSeriesGroup,
  styleType: QTMediaSeriesStyleType,
  data: QTMediaSeriesData
  ...
  const isVip =
    styleType == QTMediaSeriesStyleType.QT_MEDIA_SERIES_STYLE_TYPE_VIP
  const param: any = {
    //data
    initPosition: data.initPosition,
...
export function buildMediaSeriesContentHeight(
  type: QTMediaSeriesType,
  group: QTMediaSeriesGroup,
  styleType: QTMediaSeriesStyleType,
  data: QTMediaSeriesData
...
export function buildMediaSeriesHeight(
  type: QTMediaSeriesType,
  group: QTMediaSeriesGroup,
  styleType: QTMediaSeriesStyleType,
  data: QTMediaSeriesData
...
export function buildSeriesLeftRightHeight(
  type: QTMediaSeriesType,
  group: QTMediaSeriesGroup,
  styleType: QTMediaSeriesStyleType,
  data: QTMediaSeriesData,
...
export function buildSeriesNumberData(
  page: number,
  pageSize: number,
  dataArray: Array<QTMediaSeries>
): Array<QTMediaSeries> {
  ...
  if (dataArray.length > 0) {
    ...
    dataArray.forEach((item, index) => {
      const title = page * pageSize + index + 1
      const title2 = title > 9 ? `${title}` : `0${title}`
...
export function buildSeriesTextData(
  page: number,
  pageSize: number,
  dataArray: Array<QTMediaSeries>
): Array<QTMediaSeries> {
...
export function buildSeriesLeftRightData(
  page: number,
  pageSize: number,
  dataArray: Array<QTMediaSeries>
): Array<QTMediaSeries> {
...
export function buildSeriesTopDownData(
  page: number,
  pageSize: number,
  dataArray: Array<QTMediaSeries>
): Array<QTMediaSeries> {
